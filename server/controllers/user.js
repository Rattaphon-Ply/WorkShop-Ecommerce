    ...
    try {
        ...
        for(const item of cart){
            ...
            if(!product || item.count > product.quantity){
                outOfStock.push(product?.title || 'สินค้าที่ไม่ทราบชื่อ')
                // return res.status(400).json({ ok: false, message: `ขออภัย. สินค้า ${product?.title || 'product'} หมด` })
            }
        }

        if (outOfStock.length > 0) {
            return res.status(400).json({ ok: false, messages: outOfStock.map(title => `ขออภัย. สินค้า ${title} หมด`) })
        }
...
exports.saveOrder = async(req,res)=> {
    try {
        ...
        if(!userCart || userCart.products.length === 0) {
            return res.status(400).json({ ok: false, messaage: "Cart is Empty" })
        }
